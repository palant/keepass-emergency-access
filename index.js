...
async function readDatabase(args)
{
  ...
  while (true)
  {
    ...
    if (!actual_password)
    {
      actual_password = (await inquirer.prompt([{
        type: "password",
        name: "password",
    ...
    {
      if (error instanceof kdbxweb.KdbxError && error.code == "InvalidKey")
      {
        console.warn("This doesn't seem to be the correct password.");
        if (!args.password)
          continue;
      }
      throw error;
...
async function getDatabaseKey(credentials, database)
{
...
{
  let template_file = path.join(path.dirname(url.fileURLToPath(import.meta.url)), "template.html");
...
function printKey(key)
{
  ...
  for (let line of encoded.match(/.{14}/g))
    console.log(`${line.slice(0, 7)}-${line.slice(7)}`);
}

function printKeys(args, compositeKey)
{
  ...
  console.log(`Key ${args.num_keys}`);
  printKey(compositeKey);
  console.log();
  compositeKey.fill(0);
}
